...
const SideDrawerItem = ({ children, dropdown, icon, href = '', items, path = '', sideDrawerToggle, exact = true, select, selected, id }) => {
    ...
    let content;
    if (!dropdown) content = <>
        <NavLink onClick={onSideDrawerToggle} exact={exact} className="SideDrawerItem d-sm-none nav-link pl-3 text-large text-300 text-white" activeClassName="active bg-soft-10 py-3" to={href}>
            <FontAwesomeIcon fixedWidth icon={icon} className="mr-3 text-yellow" /> <span>{children}</span>
        </NavLink>
        <NavLink onClick={onSelect} exact={exact} className="SideDrawerItem nav-link d-none d-sm-block pl-3 text-large text-300 text-white" activeClassName="active bg-soft-10 py-3" to={href}>
            <FontAwesomeIcon fixedWidth icon={icon} className="mr-3 text-yellow" /> <span>{children}</span>
        </NavLink>
    </>;
    else {
        ...
        content = (
            <div>
                <Button color="link" onClick={toggle} className={`SideDrawerItem nav-link d-block w-100 pl-3 text-large text-300 rounded-0 text-left text-white position-relative ${match ? 'active bg-soft-20 py-3' : ''}`} style={match ? { paddingRight: 60 } : {}}>
                    ...
                    <span>{children}</span>
                    <FontAwesomeIcon fixedWidth icon={faAngleDown} className={`position-absolute angle-down text-yellow ${isSelected ? 'open' : ''}`} style={{ right: 16, top: '50%' }} />
                </Button>
                <Collapse isOpen={isSelected} className="pl-3 bg-brokenblue-20">
